...
std::unique_ptr<SDLGameObject> TowerFactory::createShadowTower(std::string towerTypeID)
{
	if (std::unique_ptr<GameObject>	towerObject = TheGameObjectFactory::Instance()->create(towerTypeID))
	{
		if (std::unique_ptr<SDLGameObject> shadowObject = std::unique_ptr<SDLGameObject>(dynamic_cast<SDLGameObject*>(towerObject.release())))
		{
